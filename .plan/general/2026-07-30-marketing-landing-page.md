...
| 사례 | 배울 점 |
|---|---|
| Sentry Unity | 설치 안내는 Unity Package Manager용 git URL 한 줄만 노출하고, 나머지 설정은 Unity 창(Tools 메뉴)으로 넘긴다 |
| PostHog | 히어로에 CTA 두 개(가입, 명령 한 줄)와 "사람과 이야기하기". 기능은 나열하되 첫 화면은 짧다 |
| Resend | 시작 문서를 라우팅 허브로 두고 실제 코드는 언어별 하위 문서로 미룬다 |
| Regression Games | 장르별 활용 사례를 따로 묶는다. 투자사 로고로 신뢰를 만든다 |

여기서 가져온 원칙:

- **첫 화면에 코드는 최대 한 줄.** 붙여 넣을 주소만 보이고, 발급 API 호출과 코드 연결은 접어 둔다.
- **설치는 3단계 이하.** 패키지 추가 → 프로젝트 연결 → 실행 확인.
- **내부 용어는 홍보 문구에서 뺀다.** 씬 블록, WebSocket 업그레이드, sdkId 같은 말은 `/how-it-works`나 접힌 상세에만 둔다. 랜딩에서는 "화면 상태", "연결 키"로 쓴다.
- **한국어는 합니다체.** 명령형(“확인해라”)은 홍보 문구에 쓰지 않는다.
- **신뢰 장치가 없다.** 고객 로고도 인증도 없으므로, 지금 동작하는 범위를 정확히 밝히는 것으로 대신한다.
